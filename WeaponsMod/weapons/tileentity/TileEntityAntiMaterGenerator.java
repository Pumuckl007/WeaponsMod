...
public class TileEntityAntiMaterGenerator extends TileEntityPowerBase implements IInventory {
	...
	public TileEntityAntiMaterGenerator() {

		super(false);
		inventory = new ItemStack[2];
		oldInventory = new ItemStack[2];
		rand = new Random();
		this.MAXPOWER = 1000000000;
	}
	...
	public void openChest() {
		...
		worldObj.addBlockEvent(xCoord, yCoord, zCoord, Weapons.sicurityStorage.blockID, 1, CommonUtils.booleanToInt(this.ison));
		worldObj.addBlockEvent(xCoord, yCoord, zCoord, Weapons.sicurityStorage.blockID, 6, power);
		for(int i = 0; i < TileEntityAntiMaterGenerator.INVENTORY_SIZE - 1; i ++){
			if(inventory[i]!= null){
	...
	private int getFuel(){
		int fuel;
		if(inventory[TileEntityAntiMaterGenerator.FUEL_SLOT] != null){
			switch(inventory[TileEntityAntiMaterGenerator.FUEL_SLOT].itemID){
				case(351):{
					if(inventory[TileEntityAntiMaterGenerator.FUEL_SLOT].getItemDamage() == 4){
						fuel = 5;
						break;
					}
					else{
	...
	public void updateEntity() {
		...
		if(addingfuel > 0){
			if(inventory[TileEntityAntiMaterGenerator.FUEL_SLOT].stackSize >= 1){
				this.addPower(addingfuel);
				inventory[TileEntityAntiMaterGenerator.FUEL_SLOT].stackSize --;
				worldObj.addBlockEvent(xCoord, yCoord, zCoord, Weapons.sicurityStorage.blockID, 6, power);
				int stack = TileEntityAntiMaterGenerator.FUEL_SLOT;
				int id = inventory[stack].itemID;
				int count = inventory[stack].stackSize;
				int damage = inventory[stack].getItemDamage();
				worldObj.addBlockEvent(xCoord, yCoord, zCoord, Weapons.sicurityStorage.blockID, 2, stack);
				...
				worldObj.addBlockEvent(xCoord, yCoord, zCoord, Weapons.sicurityStorage.blockID, 7, TileEntityAntiMaterGenerator.FUEL_SLOT);
			}
		}
		super.updateEntity();
		if (++ticksSinceSync % 20 == 0) {
			...
			if(!this.worldObj.isRemote){
				worldObj.addBlockEvent(xCoord, yCoord, zCoord, Weapons.sicurityStorage.blockID, 1, CommonUtils.booleanToInt(ison));
				for(int i = 0; i < TileEntityAntiMaterGenerator.INVENTORY_SIZE; i ++){
					if(inventory[i]!= null){
						if(oldInventory[i] == null){
							int stack = i;
							int id = inventory[stack].itemID;
		...
		ticksSinceSync ++;
		for (int j20 = 0; j20 < 5; ++j20)
		{
				@SuppressWarnings("unused")
				Random rand;
				this.worldObj.spawnParticle("tilecrack_" + 119 + "_" + this.worldObj.getBlockMetadata(this.xCoord, this.yCoord, this.zCoord), this.xCoord + (double)this.rand.nextFloat(), this.yCoord + (double)this.rand.nextFloat(), this.zCoord + (double)this.rand.nextFloat(),0, 1.5D,  0);					
		}
	}
